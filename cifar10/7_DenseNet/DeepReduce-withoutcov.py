...
if __name__ == "__main__":
    ...
    path_res = os.getcwd() + '/result/DeepReduce/'
    ...
    tt,tlog = DR_kl(NDist,outputs_ori,NIndex,NMap,path_res + 'DR-%s-%s.iteration.fixsize'%(KN,input_file))
    #tt = hsg(mapdict_ori,tnum_ori)
    #tt = hsg(testdict,7)
    f = open(path_res + 'DR-%s-%s.result.fixsize'%(KN,input_file),'w')
    f.write(str(tt) + '\n')
    f.close()
    print('length : %d, %s' %(len(tt),tt))
    
    f = open(path_res + 'DR-%s-%s.log.fixsize'%(KN,input_file),'w')
    for item in tlog:
        f.write(str(item) + '\n')
    f.close()
