...
if __name__ == "__main__":
    ...
    #mapdict_ori = {}
    mapdict_ori = [[] for i in range(nnum_ori)]
    for i in tqdm(range(nnum_ori)):
        #mapdict_ori[i] = []
        for j in range(tnum_ori):
            if cov[i][j] == '1':
                mapdict_ori[i].append(j)
        #mapdict_ori[i] = list(set(mapdict_ori[i]))
    del cov
    mapdict_np = np.array(mapdict_ori)
    del mapdict_ori
    is_sparse = []
    for item in mapdict_np:
        is_sparse.append(len(item))
    print(' %s * %s'%(tnum_ori,nnum_ori))
    print(np.mean(is_sparse))
    uni_list = []
    for item in mapdict_np:
        if item in uni_list:
            pass
    ...
    tt,tlog = DR_kl(mapdict_ori,tnum_ori,NDist,outputs_ori,NIndex,NMap,path_res + 'DRcov-%s-%s.iteration.np'%(KN,input_file))
    #tt = hsg(mapdict_ori,tnum_ori)
    #tt = hsg(testdict,7)
    f = open(path_res + 'DRcov-%s-%s.result.np'%(KN,input_file),'w')
    f.write(str(tt) + '\n')
    f.close()
    print('length : %d, %s' %(len(tt),tt))
    
    f = open(path_res + 'DRcov-%s-%s.log.np'%(KN,input_file),'w')
    for item in tlog:
        f.write(str(item) + '\n')
    f.close()
