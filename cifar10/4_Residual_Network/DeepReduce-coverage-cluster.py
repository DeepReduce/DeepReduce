...
if __name__ == "__main__":
    ...
    cov_ori = readFile(path + 'Cov/activeneuron/'+ str(threshold) + 'ase/test_cov')
    outputs_ori = readFile(os.getcwd() + '/input/' + input_file)
    tnum_ori = len(cov_ori[0])
    nnum_ori = len(cov_ori)
    cov_before_cluster = []
    for i in range(nnum_ori):
        temp_list = [False] * tnum_ori
        for j in range(tnum_ori):
            if cov_ori[i][j] == '1':
                temp_list[j] = True
        cov_before_cluster.append(copy.deepcopy(temp_list))
        del temp_list
    cov_before_cluster = np.array(cov_before_cluster)
    clf = KMeans(n_clusters=1000)
    cov_after_cluster = clf.fit(cov_before_cluster)
    cov = []
    for item in clf.cluster_centers_:
        temp_str = ''
        for i in item:
            if i == 1.0:
                temp_str += '1'
    ...
    tt,tlog = DR_kl(mapdict_ori,tnum_ori,NDist,outputs_ori,NIndex,NMap,path_res + 'DRcov-%s-%s-cluster.iteration'%(KN,input_file))
    #tt = hsg(mapdict_ori,tnum_ori)
    #tt = hsg(testdict,7)
    f = open(path_res + 'DRcov-%s-%s-cluster.result'%(KN,input_file),'w')
    f.write(str(tt) + '\n')
    f.close()
    print('length : %d, %s' %(len(tt),tt))
    
    f = open(path_res + 'DRcov-%s-%s-cluster.log'%(KN,input_file),'w')
    for item in tlog:
        f.write(str(item) + '\n')
    f.close()
