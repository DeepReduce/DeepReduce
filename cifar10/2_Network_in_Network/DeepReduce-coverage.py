...
if __name__ == "__main__":
    ...
    tt,tlog = DR_kl(mapdict_ori,tnum_ori,NDist,outputs_ori,NIndex,NMap,path_res + 'DRcov-%s-%s.iteration.fixsize'%(KN,input_file))
    #tt = hsg(mapdict_ori,tnum_ori)
    #tt = hsg(testdict,7)
    f = open(path_res + 'DRcov-%s-%s.result.fixsize'%(KN,input_file),'w')
    f.write(str(tt) + '\n')
    f.close()
    print('length : %d, %s' %(len(tt),tt))
    
    f = open(path_res + 'DRcov-%s-%s.log.fixsize'%(KN,input_file),'w')
    for item in tlog:
        f.write(str(item) + '\n')
    f.close()
