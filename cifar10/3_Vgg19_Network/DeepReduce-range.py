...
if __name__ == "__main__":
    ...
    stime = time.time()
    tt,tlog = DR_kl(mapdict_ori,tnum_ori,NDist,outputs_ori,NIndex,NMap,path_res + 'DRcov-range-%s-%s.iteration.fixsize.time'%(KN,input_file))
    ...
    #tt = hsg(testdict,7)
    f = open(path_res + 'DRcov-range-%s-%s.result.fixsize.time'%(KN,input_file),'w')
    f.write(str(tt) + '\n')
    f.close()
    print('length : %d, %s' %(len(tt),tt))
    
    f = open(path_res + 'DRcov-range-%s-%s.log.fixsize.time'%(KN,input_file),'w')
    for item in tlog:
        f.write(str(item) + '\n')
    f.close()
