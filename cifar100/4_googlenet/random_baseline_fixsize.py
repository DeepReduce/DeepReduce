...
def random_kl(NDist,outputs,NIndex,NMap,filepath,size):
    #f = open(filepath,'w')
    RST = []
    log = []
    dnum = len(NDist)
...
if __name__ == "__main__":
    ...
    runningnumber = str(sys.argv[2])
    size = int(sys.argv[3])
    path = os.getcwd() + '/input/'
    path_res = os.getcwd() + '/result/RANDOM_fixsize/'
    ...
    tt,tlog = random_kl(NDist,outputs_ori,NIndex,NMap,path_res + 'random-%s-%s.iteration'%(KN,input_file), size)
    #tt = hsg(mapdict_ori,tnum_ori)
    #tt = hsg(testdict,7)
    f = open(path_res + 'random-%s-%s.result'%(KN,input_file) + '_' + runningnumber,'w')
    f.write(str(tt) + '\n')
    f.close()
    print('length : %d, %s' %(len(tt),tt))
    
    f = open(path_res + 'random-%s-%s.log'%(KN,input_file) + '_' + runningnumber,'w')
    for item in tlog:
        f.write(str(item) + '\n')
    f.close()
