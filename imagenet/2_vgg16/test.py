...
def DR_kl(NDist,outputs,NIndex,NMap,filepath):
    f = open(filepath,'w')
    RST = []
    log = []
    dnum = len(NDist)
...
if __name__ == "__main__":
    ...
    outputs_ori = readFile(os.getcwd() + '/input/last_output')
    #tnum_ori = len(cov[0])
    #nnum_ori = len(cov)
    ...
    tt,tlog = DR_kl(NDist,outputs_ori,NIndex,NMap,path_res + 'DR-%s.iteration'%KN)
    #tt = hsg(mapdict_ori,tnum_ori)
    #tt = hsg(testdict,7)
    f = open(path_res + 'DR-%s.result'%KN,'w')
    f.write(str(tt) + '\n')
    f.close()
    print('length : %d, %s' %(len(tt),tt))
    
    f = open(path_res + 'DR-%s.log'%KN,'w')
    for item in tlog:
        f.write(str(item) + '\n')
    f.close()
    '''
    print(calculate_acc(predict,list(range(50000))))
