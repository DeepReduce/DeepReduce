...
def SelectTest_cross_entropy(SIZE,LIST,mapdict,curdict,tnum,marked,MAX_CARD):
    count = [0]*tnum
    maxcount = 0
    ...
    if len(testlist) == 1:
        ...
        return SelectTest_cross_entropy(cc[ccindex+1],testlist,mapdict,curdict,tnum,marked,MAX_CARD)
...
def cal_ce(ds,dt,snum):
    return sum(-dt*log(ds/snum))/len(ds)

def cal_kl(ds,dt,snum):
    return sum(dt*log(dt/(ds/snum)))/len(ds)
...
def fse(fseter,NDist,outputs,NIndex,NMap):
    ...
    #print('nnum : %s'%dnum)
    for i in range(dnum):
        RSTDist.append([1e-15] * KN)
    RSTDist = np.array(RSTDist)
    #print(NDist)
    #print(cal_ce(RSTDist,NDist,len(RST)))
    terminal_ce =  float('inf')
    tempkl = float('inf')
    RS = random.sample(range(test_num),30)
    ...
    while(tempkl > fseter):
        q = 5
        examples = []
        remain = list(set(range(test_num)) - set(RST))
        #print('%s : %s'%(len(remain),q))
        for i in range(300): 
        ...
        for i in range(300):
            ...
            #ttime1 = time.time()
            tempce = cal_ce(exdist,NDist,len(RST)+len(example))
            #ttime2 = time.time()
            #print('%s - %s'%(ttime1-ttime,ttime2-ttime1))
            #print(tempce)
            #print(tempce2)
            #print(tempce2 - tempce)
            #input('check...')
            #print(RSTDist)
            if tempce < mince:
                mince = tempce
        ...
        #len_rst = len(RST)
        #tempkl2 = sum(-NDist*log(RSTDist/len(RST)))
        tempkl = cal_kl(RSTDist,NDist,len(RST))
        #ttime2 = time.time()
        #print(ttime1-ttime,ttime2-ttime1)
        #print(tempkl2-tempkl1)
        #input('check...')
        #print(minkl)
        #print(minindex)
        #print('%s : %s '%(tempkl,len(RST)))
...
if __name__ == "__main__":
    ...
    fseter = float(sys.argv[1])
    runN = int(sys.argv[2])
    path1 = os.getcwd() + '/input/'
    path2 = os.getcwd() + '/result/FSE_new/'
