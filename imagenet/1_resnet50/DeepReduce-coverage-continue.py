...
# RST : selected set of test instance
def hgs_slice(mapdict,RST,tnum):
    ...
    #List = []
        ...
        while True:
            ...
            if len(List)== 0:
                break
            next_test = SelectTest_dlr(cur_card,List,mapdict,curdict,tnum,marked,MAX_CARD,5)
            #print('*************************')
            #print('list : %s'%List)
...
def DR_kl(RST,tnum,NDist,outputs,NIndex,NMap,filepath):
    ...
    RSTDist = update_dist(RST,RSTDist,outputs,NIndex)
...
def getCandidate_new(RSTDist,NDist,NMap,tnum,RST,NIndex,outputs):
    ...
    maxsim = 0
    remain = list(set(range(50000))- set(RST))
...
if __name__ == "__main__":
    ...
    path_res = os.getcwd() + '/result/DeepReduce/' + str(threshold) + '/'
    if os.path.exists(path_res) == False:
        os.makedirs(path_res)
    #cov = readFile(path + 'Cov/activeneuron/'+ str(threshold) + 'ase/test_cov')
    outputs_ori = readFile(os.getcwd() + '/input/' + input_file)
    #global slice_num
    #slice_num = min(slice_num,len(cov))
    tnum_ori = len(outputs_ori)
    #nnum_ori = len(cov)
    ...
    for i in tqdm(range(slice_num)):
        ...
        for j in range(tnum_ori):
            if cov[i][j] == '1':
                ...
                #mapdict_ori[i] = mapdict_ori[i]|set([j])
        mapdict_ori[i] = set(copy.deepcopy(mapdict_ori[i]))
    for i in range(slice_num):
        del cov[0]
    RST = hgs_slice(mapdict_ori,[],tnum_ori)
    #RST_2 = hgs_slice(mapdict_ori,[],tnum_ori)
    #print(RST)
    #for i in range(20):
    #    print(hgs_slice(mapdict_ori,[],tnum_ori))
    del mapdict_ori
    while len(cov) > 0:
        print('hgs slice (next) ..., %s'%len(cov))
        mapdict_ori = {}
        dict_index = 0
        while len(mapdict_ori) < slice_num and len(cov) >0:
            temp_detected_test = []
            for j in range(tnum_ori):
                if cov[0][j] == '1':
                    temp_detected_test.append(j)
            if len(set(temp_detected_test) & set(RST)) == 0:
                mapdict_ori[dict_index] = set(copy.deepcopy(temp_detected_test))
                del temp_detected_test
                dict_index += 1
            del cov[0]
        RST = hgs_slice(mapdict_ori,RST,tnum_ori)
    ...
    #print(NMap)
    #input('check...')
    RST = eval(readFile(path_res + 'DRcov-%s-%s.result.slice'%(KN,input_file))[0])
    #tt,tlog = DR_kl(NDist,outputs_ori,NIndex,NMap,path_res + 'DR-%s-%s.iteration'%(KN,input_file))
    print('starting ...')
    tt,tlog = DR_kl(RST,tnum_ori,NDist,outputs_ori,NIndex,NMap,path_res + 'DRcov-%s-%s.iteration.continue'%(KN,input_file))
    #tt = hsg(mapdict_ori,tnum_ori)
    #tt = hsg(testdict,7)
    f = open(path_res + 'DRcov-%s-%s.result.continue'%(KN,input_file),'w')
    f.write(str(tt) + '\n')
    f.close()
    print('length : %d, %s' %(len(tt),tt))
    
    f = open(path_res + 'DRcov-%s-%s.log.continue'%(KN,input_file),'w')
    for item in tlog:
        f.write(str(item) + '\n')
    f.close()
