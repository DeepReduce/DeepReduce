# 存的：行是每一层，列是该层的神经元，1为曾经为topk,0为没有曾经为topk
import keras
...
#             if g == 0:
#                 newn_layer = [0] * intermediate.shape[-1]    # 这么多个神经元
#                 Top_k_Neuron_Coverage.append(newn_layer)
#             cur_layer = []
#             for num_neuron in range(intermediate.shape[-1]):  # 一个神经元
#                 value = np.mean(intermediate[..., num_neuron])
#                 # value = np.max(intermediate[..., num_neuron])
#                 cur_layer.append(value)
#             indexes = np.argpartition(cur_layer, -k)[-k:]  # 前k大的index
#             # indexes = np.argmax(cur_layer)
#             for index in indexes:
#                 Top_k_Neuron_Coverage[i][index] = 1
#             # Top_k_Neuron_Coverage[i][indexes] = 1
