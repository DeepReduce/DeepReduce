...
def build_model():
    ...
    model.add(Conv2D(int(4*change[0]), (5, 5), activation='relu', padding='same', input_shape=input_shape))
    model.add(MaxPooling2D(pool_size=(2, 2)))

    model.add(Conv2D(int(12*change[1]), (5, 5), activation='relu', padding='same'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
...
if __name__ == '__main__':
    ...
    # save model
    path = changetype + 'neuronnumber-lenet-1/' + str(changeindex)
    if os.path.exists(path) == False:
        os.makedirs(path)
    model.save(path + '/lenet-1.h5')
