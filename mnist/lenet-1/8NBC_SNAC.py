...
if __name__ == '__main__':
    ...
    filename = 'lenet-1_' + str(times) + '.h5'
    model1 = load_model('Cov/model/' + filename)
    ...
    noutput = get_all_output('Cov/model/' + filename)
    ...
    f = open(path1 + '2NBC' + '/max/' + 'result', 'a')
    f.write(ss1 + '\n')
    f.close()
    ...
    f = open(path1 + '3SNAC' + '/max/' + 'result', 'a')
    f.write(ss2 + '\n')
    f.close()

    f = open('Cov/2NBC/max/' + str(times), 'w')
    for i in tqdm(range(len(Cov_NBC))):
        f.write(Cov_NBC[i] + '\n')
    f.close()

    t = open('Cov/3SNAC/max/' + str(times), 'w')
