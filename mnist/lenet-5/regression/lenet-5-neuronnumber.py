...
def build_model():
    ...
    model.add(Flatten())
    model.add(Dense(int(change[2]*120), activation='relu'))
    model.add(Dense(int(change[3]*84), activation='relu'))
    model.add(Dense(10))
...
if __name__ == '__main__':
    ...
    # save model
    path = changetype + 'neuronnumber-lenet-5/' + str(changeindex)
    if os.path.exists(path) == False:
        os.makedirs(path)
    model.save(path + '/lenet-5.h5')
